...
export class DataRetentionService {
  ...
  async getAdditionalData(): Promise<AdditionalData> {
    ...
    const data = await this.addonService.getAddonApiCall(config.AddonUUID, 'api', `get_addon_data`).toPromise();
    if(data.success) {
  ...
  async updateAdditionalData(additionalData: AdditionalData) {
    ...
    // })
    await this.addonService.postAddonApiCall(config.AddonUUID, 'api', `update_addon_data`, currentData).toPromise();
  }
  ...
  updateSystemData(body: any, successFunc, errorFunc = null) {
   ...
   openDialog(title = 'Modal Test', content, buttons,
    ...
    async getTypes(successFunc = null, errorFunc = null){
      let types:KeyValuePair<string>[] = [];
      await this.httpService.getPapiApiCall('/meta_data/activities/types').subscribe(
            async (activityTypes) => {
                if (activityTypes) {
    ...
    async getReportToken(){
        const token = await this.papiClient.addons.api.uuid(config.AddonUUID).async().file('api').func('get_archive_report').post({retry:25});
    ...
    async updateCodeJob(job: CodeJob) {
        return await this.addonService.postAddonApiCall(config.AddonUUID, 'api', 'update_code_job', job).toPromise();
    }
}
